...
public class GeneticAlgorithm {
     ...
     * @throws Exception
     */
    public List<Integer> listByGeneticAlgorithm(List<ArticleInfo> articleInfoList, Map<Integer, Integer> userStar)
            throws Exception {
        //获得种群初始化参数
        ...
        while (true) {
            ...
            if ((adaptability >= (allAdaptability * (curOfIndividuals / numOfIndividuals) * times))
                    && (generation > numOfIndividuals / (curOfIndividuals * times))) {
                //停止条件
                break;
            }
            //交配繁殖下一代
    ...
    public double getAdaptability(List<Boolean[]> genes, List<ArticleInfo> articleInfoList,
                                  Map<Integer, Integer> userStar) throws Exception {
        double result = 0;
        int genesLen = genes.size();
        for (int i = 0; i < genesLen; i++) {
            Boolean[] gene = genes.get(i);
            int num = getGeneId(gene);
            result += userStar.get(articleInfoList.get(num).getArticleType());
        }
        return result;
    }
